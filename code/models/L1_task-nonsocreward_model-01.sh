...
maindir="/data/projects/rut_sr"
TASK=nonsocialreward
cd $maindir
...
# to do - add logging option if running through a second time
# set input and output and adjust for ppi

MAINOUTPUT=${maindir}/data/results/lowerLv_Results/sub-${sub}
DATA=${maindir}/data/derivatives/fmriprep/sub-${sub}/func/sub-${sub}_task-${TASK}_run-0${run}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz 
CONFOUNDEVS=${maindir}/data/derivatives/confounds/sub-${sub}/sub-${sub}_task-learning_run-0${run}_desc-fslConfounds.tsv
EVDIR=${maindir}/data/EVfiles/3col/sub-${sub}/

PMODDIR=${maindir}/data/EVfiles/3col/sub-${sub}/
...
ITEMPLATE=/data/projects/istart-socDoors/code/templates/design.fsf
...
if [ "$ppi" == "0" ]; then
	...
	MASK=${maindir}/code/masks/${ppi}_func.nii.gz
